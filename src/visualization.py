...
def visualize_bbox(img: ndarray, bbox: COCOBoundingBox, class_name: str, colour: tuple[int]=RED, thickness: int=2) -> ndarray:
    ...
    cv2.rectangle(img, (x_min, y_min), (x_max, y_max), color=colour, thickness=thickness)

    ((text_width, text_height), _) = cv2.getTextSize(class_name, cv2.FONT_HERSHEY_SIMPLEX, 0.35, 1)
    cv2.rectangle(img, (x_min, y_min - int(1.3 * text_height)), (x_min + text_width, y_min), WHITE, -1)
    cv2.putText(
        img,
        text=class_name,
...
def visualize(image: ndarray, bboxes: list[COCOBoundingBox], category_ids: list[int],
    ...
    for bbox, category_id in zip(bboxes, category_ids):
        ...
        img = visualize_bbox(img, bbox, class_name, colour)
